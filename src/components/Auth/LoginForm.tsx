...
const LoginForm: React.FC<LoginFormProps> = ({
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      console.log('🔍 LOGIN DEBUG - User from login():', JSON.stringify(loggedInUser, null, 2));
      console.log('🔍 LOGIN DEBUG - User roles from login():', loggedInUser?.roles);
      
      if (loggedInUser && loggedInUser.roles) {
        // Filter roles (không phân biệt hoa thường)
        const validRoles = loggedInUser.roles.filter(role => 
          ['user', 'photographer', 'locationowner'].includes(role.toLowerCase())
        );
        ...
        if (validRoles.length === 1) {
          ...
          console.log('🔍 Multiple roles detected, navigating to RoleSelection');
