...
const sendMessage = useCallback(async (
  ...
  try {
    ...
    if (conversation.otherParticipant?.userId) {
      ...
      console.log('✅ Using recipient from conversation.otherParticipant:', recipientId);
      ...
      console.log('🔍 Looking for recipient in participants:', {
        currentUserId,
      ...
      if (recipient) {
        ...
        throw new Error('Could not find recipient - all participants are current user');
      }
    } else {
      // ✅ FIX 3: Last resort - use first participant if available
      if (conversation.participants && conversation.participants.length > 0) {
        recipientId = conversation.participants[0].userId;
        console.log('⚠️ Using first participant as recipient:', recipientId);
      } else {
        console.error('❌ No participants found in conversation:', conversation);
        throw new Error('No recipient found - conversation has no participants');
      }
    }
    ...
    console.log('💬 Sending message with validated recipient:', {
      conversationId,
      recipientId,
    ...
    console.log('📤 ChatService sendMessage response:', response);
...
const markMessagesAsRead = useCallback(async (messageIds?: number[]): Promise<void> => {
  ...
  try {
    ...
    if (messageIds) {
      ...
      messagesToMark = messages.filter(msg => messageIds.includes(msg.messageId));
    } else {
    ...
    console.log('👀 Marking messages as read:', {
      ...
      currentUserId,
      messageIds: messagesToMark.map(m => ({ id: m.messageId, senderId: m.senderId }))
    });
    ...
    const markPromises = messagesToMark.map(async (message) => {
      try {
        ...
        if (message.senderId === currentUserId) {
          console.log('📝 Skipping own message:', message.messageId);
          return;
        }

        console.log('👀 Marking message as read:', message.messageId, 'from sender:', message.senderId);
        await chatService.markMessageAsRead(message.messageId);
        console.log('✅ Message marked as read successfully:', message.messageId);
    ...
    setMessages(prev => prev.map(msg => {
      const shouldUpdate = messagesToMark.some(marked => 
    ...
    console.log('✅ Messages marked as read process completed');
