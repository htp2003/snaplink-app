...
export const useAvailability = (options: UseAvailabilityOptions = {}) => {
  const { photographerId, autoFetch = false, enableRealTimeUpdates = false } = options;
  ...
  const [availablePhotographers, setAvailablePhotographers] = useState<AvailablePhotographersResponse>({ photographers: [] });
  const [checkResult, setCheckResult] = useState<CheckAvailabilityResponse | null>(null);
  const [searchLoading, setSearchLoading] = useState(false);

  // ===== VALIDATION =====
  const validateAvailabilityForm = useCallback((formData: AvailabilityFormData): AvailabilityValidationErrors => {
    ...
    if (formData.startTime && formData.endTime) {
      ...
      const existingSlots = availabilities.filter(
        av => av.photographerId === formData.photographerId && 
  ...
  const validateBulkAvailabilityForm = useCallback((formData: BulkAvailabilityFormData): AvailabilityValidationErrors => {
    const errors: AvailabilityValidationErrors = {};
    ...
    const hasEnabledDay = Object.values(formData.schedule).some(daySchedule => 
      ...
      if (daySchedule && daySchedule.isEnabled) {
        daySchedule.slots.forEach((slot, index) => {
          if (!slot.startTime || !slot.endTime) {
            errors.general = `${DAY_NAMES[parseInt(dayKey) as DayOfWeek]}: Vui lòng điền đầy đủ thời gian`;
            return;
          }
          ...
          if (endTime <= startTime) {
            errors.general = `${DAY_NAMES[parseInt(dayKey) as DayOfWeek]}: Giờ kết thúc phải sau giờ bắt đầu`;
            return;
          }
          ...
          for (let i = index + 1; i < daySchedule.slots.length; i++) {
            ...
            if (availabilityService.isTimeOverlap(
              slot.startTime, slot.endTime,
              otherSlot.startTime, otherSlot.endTime
            )) {
              errors.general = `${DAY_NAMES[parseInt(dayKey) as DayOfWeek]}: Các khung giờ bị trùng lặp`;
              return;
            }
          }
        });
  ...
  const createAvailability = useCallback(async (
    ...
    try {
      ...
      console.log('🔧 Hook: Creating availability with data:', data);
      ...
      console.log('✅ Hook: Availability created successfully');
      return response;
    } catch (err) {
  ...
  const createBulkAvailability = useCallback(async (
    ...
    try {
      ...
      console.log('🔧 Hook: Creating bulk availability with data:', data);
      ...
      console.log('✅ Hook: Bulk availability created successfully');
      return response;
    } catch (err) {
      const errorMessage = err instanceof Error ? err.message : 'Không thể tạo lịch làm việc';
      setError(errorMessage);
  ...
  const getAvailabilityById = useCallback(async (availabilityId: number): Promise<AvailabilityResponse | null> => {
    try {
      setLoading(true);
      setError(null);

      const response = await availabilityService.getAvailabilityById(availabilityId);
      setCurrentAvailability(response);
      return response;
    } catch (err) {
      const errorMessage = err instanceof Error ? err.message : 'Không thể lấy thông tin lịch làm việc';
      setError(errorMessage);
  ...
  const updateAvailability = useCallback(async (
    ...
    try {
      ...
      const response = await availabilityService.updateAvailability(availabilityId, data);
      
      setCurrentAvailability(response);
      setAvailabilities(prev => prev.map(availability => 
        availability.availabilityId === availabilityId || availability.id === availabilityId 
          ? response 
          : availability
      ));
  ...
  const deleteAvailability = useCallback(async (availabilityId: number): Promise<boolean> => {
    ...
    try {
      ...
      setAvailabilities(prev => prev.filter(availability => 
        availability.availabilityId !== availabilityId && availability.id !== availabilityId
      ));

      if (currentAvailability && 
          (currentAvailability.availabilityId === availabilityId || currentAvailability.id === availabilityId)) {
  ...
  const updateAvailabilityStatus = useCallback(async (
    ...
    try {
      ...
      await availabilityService.updateAvailabilityStatus(availabilityId, status);
  ...
  const fetchPhotographerAvailability = useCallback(async (photographerIdParam: number) => {
    try {
      setLoading(true);
      setError(null);

      const response = await availabilityService.getPhotographerAvailability(photographerIdParam);
      ...
      const errorMessage = err instanceof Error ? err.message : 'Không thể lấy lịch làm việc';
      setError(errorMessage);
  ...
  const fetchPhotographerAvailabilityDetails = useCallback(async (
    ...
    try {
      ...
      const response = await availabilityService.getPhotographerAvailabilityDetails(photographerIdParam);
      return response;
    } catch (err) {
      const errorMessage = err instanceof Error ? err.message : 'Không thể lấy chi tiết lịch làm việc';
      setError(errorMessage);
      console.error('❌ Hook: Error in fetchPhotographerAvailabilityDetails:', err);
      return [];
    } finally {
      setLoading(false);
    }
  ...
  const deleteAllPhotographerAvailability = useCallback(async (photographerIdParam: number): Promise<boolean> => {
    if (deleting) return false;
    ...
    try {
      ...
      await availabilityService.deleteAllPhotographerAvailability(photographerIdParam);
      ...
      const errorMessage = err instanceof Error ? err.message : 'Không thể xóa toàn bộ lịch làm việc';
      setError(errorMessage);
      console.error('❌ Hook: Error in deleteAllPhotographerAvailability:', err);
      return false;
    } finally {
      setDeleting(false);
    }
  ...
  const fetchAvailabilityByDay = useCallback(async (dayOfWeek: DayOfWeek) => {
    try {
      ...
      const response = await availabilityService.getAvailabilityByDay(dayOfWeek);
      return response;
    } catch (err) {
      const errorMessage = err instanceof Error ? err.message : 'Không thể lấy lịch theo ngày';
      setError(errorMessage);
  ...
  const searchAvailablePhotographers = useCallback(async (params: GetAvailablePhotographersParams) => {
    try {
      setSearchLoading(true);
      setError(null);

      const response = await availabilityService.getAvailablePhotographers(params);
      setAvailablePhotographers(response);
      return response;
    } catch (err) {
      const errorMessage = err instanceof Error ? err.message : 'Không thể tìm kiếm photographer';
      setError(errorMessage);
  ...
  const checkAvailability = useCallback(async (data: CheckAvailabilityRequest) => {
    try {
      setChecking(true);
      setError(null);
  ...
  const refreshCurrentAvailability = useCallback(async () => {
    if (currentAvailability?.availabilityId || currentAvailability?.id) {
      const availabilityId = currentAvailability.availabilityId || currentAvailability.id!;
  ...
  const buildBulkAvailabilityRequest = useCallback((
    formData: BulkAvailabilityFormData
  ): BulkAvailabilityRequest => {
  ...
  const getSlotsByDay = useCallback((dayOfWeek: DayOfWeek): TimeSlot[] => {
  ...
  return {
    ...
    // ===== REFRESH UTILITIES =====
    refreshPhotographerAvailability: () => photographerId && fetchPhotographerAvailability(photographerId),
