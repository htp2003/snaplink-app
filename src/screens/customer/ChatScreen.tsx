...
const ChatScreen = () => {
  ...
  const handleProfilePress = useCallback(() => {
    if (otherUser) {
      // Navigate to user profile or photographer profile
      console.log('👤 View profile for user:', otherUser.userId);
      // navigation.navigate('UserProfile', { userId: otherUser.userId });
    }
  }, [otherUser]);
  ...
  }, [navigation, title, handleProfilePress]);
  ...
  const handleRetryMessage = useCallback(async (messageId: number) => {
    ...
    const failedMessage = messages.find((msg: Message) => msg.messageId === messageId && msg.status === MessageStatus.FAILED);
    if (failedMessage) {
  ...
  const renderMessage = useCallback(({ item: message, index }: { item: Message; index: number }) => {
    const isCurrentUser = message.senderId === otherUser?.userId; // Simplified - should use actual current user ID
    const isLastMessage = index === messages.length - 1;
    const showAvatar = !isCurrentUser && (isLastMessage || messages[index + 1]?.senderId !== message.senderId);
    ...
    return (
      <View style={styles.messageContainer}>
        ...
        <View style={[
          ...
          {!isCurrentUser && (
            <View style={[styles.avatarContainer, !showAvatar && styles.avatarPlaceholder]}>
              {showAvatar && (
                <Image
                  source={{ uri: otherUser?.userProfileImage || 'https://via.placeholder.com/32x32' }}
          ...
          <View style={[
            ...
            message.status === MessageStatus.FAILED && styles.failedMessageBubble
          ]}>
            <Text style={[
              styles.messageText,
              ...
              {isCurrentUser && (
                <View style={styles.messageStatusContainer}>
                  ...
                  {message.status === MessageStatus.READ && (
                    <Text style={[styles.messageStatusIcon, styles.readStatusIcon]}>✓✓</Text>
                  )}
                  {message.status === MessageStatus.FAILED && (
                    <TouchableOpacity onPress={() => handleRetryMessage(message.messageId)}>
  ...
  }, [messages, otherUser, handleRetryMessage]);
  ...
  const renderErrorState = useCallback(() => {
    ...
    return (
      <View style={styles.errorContainer}>
        ...
        <TouchableOpacity style={styles.errorDismissButton} onPress={handleErrorDismiss}>
    ...
    <SafeAreaView style={styles.container}>
      ...
      >
        <FlatList
          ref={flatListRef}
          data={messages}
          keyExtractor={(item) => `message_${item.messageId}_${item.localId || ''}`}
        ...
        <View style={styles.inputContainer}>
          <View style={styles.inputWrapper}>
            ...
            <TouchableOpacity 
              style={[
                styles.sendButton,
                (messageText.trim().length === 0 || sendingMessage) && styles.sendButtonDisabled
              ]}
