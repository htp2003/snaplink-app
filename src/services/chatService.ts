...
const CHAT_ENDPOINTS = {
  ...
  UPDATE_CONVERSATION: (conversationId: number) => `/api/chat/conversations/${conversationId}`,
  DELETE_CONVERSATION: (conversationId: number) => `/api/chat/conversations/${conversationId}`,
  GET_CONVERSATION_MESSAGES: (conversationId: number) => `/api/chat/conversations/${conversationId}/messages`,
  
  // Participants
  ADD_PARTICIPANT: (conversationId: number) => `/api/chat/conversations/${conversationId}/participants`,
  REMOVE_PARTICIPANT: (conversationId: number) => `/api/chat/conversations/${conversationId}/participants`,
  GET_PARTICIPANTS: (conversationId: number) => `/api/chat/conversations/${conversationId}/participants`,
  LEAVE_CONVERSATION: (conversationId: number) => `/api/chat/conversations/${conversationId}/leave`,
  
  // Utilities
  GET_UNREAD_COUNT: (conversationId: number) => `/api/chat/conversations/${conversationId}/unread-count`,
  IS_PARTICIPANT: (conversationId: number) => `/api/chat/conversations/${conversationId}/is-participant`,
  ...
  GET_PHOTOGRAPHER_DETAIL: (photographerId: number) => `/api/Photographer/${photographerId}`,
...
export class ChatService {
  ...
  async getConversations(page: number = 1, pageSize: number = 20): Promise<GetConversationsResponse> {
    try {
      console.log('📋 Fetching conversations:', { page, pageSize });
  ...
  async getConversationMessages(
    ...
    try {
      console.log('💬 Fetching conversation messages:', { conversationId, page, pageSize });
  ...
  async createConversation(conversationData: CreateConversationRequest): Promise<CreateConversationResponse> {
    try {
  ...
  async updateConversation(
    ...
    try {
      console.log('✏️ Updating conversation:', { conversationId, title, status });
  ...
  async deleteConversation(conversationId: number): Promise<void> {
    try {
      ...
      await apiClient.delete(CHAT_ENDPOINTS.DELETE_CONVERSATION(conversationId));
  ...
  async addParticipant(participantData: AddParticipantRequest): Promise<void> {
    try {
      ...
      if (!participantData.conversationId || participantData.conversationId <= 0) {
        throw new Error('Invalid conversation ID');
      }
  ...
  async removeParticipant(participantData: RemoveParticipantRequest): Promise<void> {
    try {
  ...
  async getParticipants(conversationId: number): Promise<ConversationParticipant[]> {
    try {
      console.log('👥 Fetching participants:', conversationId);
  ...
  async getUnreadCount(conversationId: number): Promise<number> {
    try {
      console.log('📊 Getting unread count:', conversationId);
  ...
  async isParticipant(conversationId: number): Promise<boolean> {
    try {
      console.log('👤 Checking if participant:', conversationId);
  ...
  async getDirectConversation(otherUserId: number): Promise<ConversationResponse> {
    try {
      console.log('💬 Getting direct conversation with user:', otherUserId);
  ...
  async searchPhotographers(params: SearchPhotographersParams): Promise<PhotographerSearchResult[]> {
    try {
      ...
      const photographers: PhotographerSearchResult[] = response.map(photographer => ({
        ...
        profileImage: photographer.profileImage || photographer.user?.profileImage,
        ...
        isVerified: photographer.verificationStatus === 'verified' || photographer.isVerified || false,
  ...
  async getAllPhotographers(page: number = 1, pageSize: number = 20): Promise<PhotographerSearchResult[]> {
    try {
      console.log('📋 Getting all photographers:', { page, pageSize });
      ...
      console.log('✅ All photographers fetched:', photographers.length);
      return photographers;
  ...
  async searchConversationsAndPhotographers(query: string): Promise<ChatSearchResult> {
    try {
      ...
      const filteredConversations = conversationsResponse.conversations
        ?.filter(conv => {
          const titleMatch = conv.title?.toLowerCase().includes(trimmedQuery.toLowerCase());
          const participantMatch = conv.participants?.some(p => 
            p.userName?.toLowerCase().includes(trimmedQuery.toLowerCase()) ||
            p.userFullName?.toLowerCase().includes(trimmedQuery.toLowerCase())
          );
      ...
      const photographersWithConversations = await Promise.all(
        photographers.map(async (photographer) => {
          try {
            const directConv = await this.getDirectConversation(photographer.userId);
            return {
              ...photographer,
      ...
      const result: ChatSearchResult = {
        ...
        totalResults: filteredConversations.length + photographersWithConversations.length
      };
      
      console.log('✅ Combined search results:', {
        conversations: result.conversations.length,
        photographers: result.photographers.length,
  ...
  normalizeMessageResponse(messageResponse: MessageResponse): Message {
    return {
      ...
      messageType: (messageResponse.messageType as MessageType) || MessageType.TEXT,
  ...
  normalizeConversationResponse(conversationResponse: ConversationResponse): Conversation {
    ...
    if (conversationResponse.type === ConversationType.DIRECT && participants.length === 2) {
    ...
    return {
      ...
      status: (conversationResponse.status as ConversationStatus) || ConversationStatus.ACTIVE,
      type: (conversationResponse.type as ConversationType) || ConversationType.DIRECT,
      participants: participants,
      lastMessage: conversationResponse.lastMessage ? 
        this.normalizeMessageResponse(conversationResponse.lastMessage) : undefined,
  ...
  validateConversationTitle(title: string): { isValid: boolean; error?: string } {
    if (!title || title.trim() === '') {
  ...
  getMessageStatusIcon(status: MessageStatus): string {
    switch (status) {
      ...
      case MessageStatus.FAILED: return '⚠️';
      default: return '';
    }
  }
  ...
  generateMockConversations(count: number = 10): Conversation[] {
    ...
    for (let i = 1; i <= count; i++) {
      ...
      mockConversations.push({
        conversationId: i,
        title: isGroup ? `Wedding Planning Team ${i}` : `John Photographer ${i}`,
  ...
  generateMockPhotographers(count: number = 20): PhotographerSearchResult[] {
    const specializations = ['Wedding', 'Portrait', 'Event', 'Fashion', 'Landscape', 'Street'];
    const locations = ['Ho Chi Minh City', 'Hanoi', 'Da Nang', 'Can Tho', 'Hue'];
  ...
  async testSendMessage(recipientId: number, testMessage: string = 'Test message'): Promise<{ success: boolean; message: string; data?: any }> {
    try {
      ...
      return {
        success: false,
        message: `Send message failed: ${error instanceof Error ? error.message : 'Unknown error'}`
      };
    }
  }
}
